...
class DDRDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        caption = []
        caption.append(vocab('<start>')) #Produce Uniform length captions
        caption.extend([vocab(token) for i, token in enumerate(tokens) if i < 998])
        caption.append(vocab('<end>'))
        if len(caption) < 1000:
            for i in range(1000 - len(caption)):
